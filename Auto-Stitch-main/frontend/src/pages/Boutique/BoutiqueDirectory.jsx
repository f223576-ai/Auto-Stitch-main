      ...
      <section className="multi-brands-section container">
        ...
        <div className="editorial-product-grid">
          {[
            ...
            // Cycle through real boutiques fetched from the API so the link
            // always points to a boutique that actually exists in the DB.
            const linkedBoutique = displayBoutiques.length > 0
              ? displayBoutiques[idx % displayBoutiques.length]
              : null;
            const linkId = linkedBoutique?._id || linkedBoutique?.id;
            const target = linkId ? `/boutiques/${linkId}` : '/boutiques';

            return (
              <Link to={target} key={idx} className="editorial-card">
