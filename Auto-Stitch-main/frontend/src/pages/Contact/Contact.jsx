...
export default function Contact() {
  ...
  const handleChange = (e) => {
    ...
    if (name === 'topic' && value === 'General Inquiry') {
      setForm(prev => ({ ...prev, topic: value, orderNumber: '' }));
    } else {
  ...
  const showOrderNumberField = form.topic && form.topic !== 'General Inquiry';
      ...
      <div className="contact-container">
        ...
        <div className="contact-form-wrapper">
          ...
          <form onSubmit={handleSubmit} className="minimal-contact-form">
            ...
            {/* Conditionally rendered Order Number field */}
            {showOrderNumberField && (
              <div className="minimal-group" style={{ animation: 'fadeIn 0.3s ease-out' }}>
                <label>
                  ORDER NUMBER {['Order Status', 'Returns & Exchanges'].includes(form.topic) ? '*' : '(OPTIONAL)'}
                </label>
                <input 
                  type="text" 
                  name="orderNumber" 
                  ...
                  onChange={handleChange}
                  required={['Order Status', 'Returns & Exchanges'].includes(form.topic)}
                />
              </div>
            )}
