...
// Display names/images for the spotlight strip. IDs are resolved at runtime
// from real boutiques fetched from the API (see realBoutiques state below),
// since these placeholder names don't correspond to real DB records.
const BOUTIQUES_DISPLAY = [
  { name: 'Élan', image: 'https://picsum.photos/seed/d9/200/200' },
  { name: 'Suffuse', image: 'https://picsum.photos/seed/d10/200/200' },
  { name: 'Khaadi', image: 'https://picsum.photos/seed/d1/200/200' },
  { name: 'Maria.B', image: 'https://picsum.photos/seed/d2/200/200' },
  { name: 'Sana Safinaz', image: 'https://picsum.photos/seed/d3/200/200' },
  { name: 'Ideas', image: 'https://picsum.photos/seed/d4/200/200' },
  { name: 'J.', image: 'https://picsum.photos/seed/d5/200/200' },
  { name: 'Nishat Linen', image: 'https://picsum.photos/seed/d6/200/200' },
  { name: 'Agha Noor', image: 'https://picsum.photos/seed/d7/200/200' },
  { name: 'Asim Jofa', image: 'https://picsum.photos/seed/d8/200/200' },
];
...
export default function Home({ user }) {
  ...
  // Fetch real boutiques so spotlight links point to boutiques that
  // actually exist in the DB instead of hardcoded placeholder IDs.
  useEffect(() => {
    const fetchBoutiques = async () => {
      try {
        const { data } = await axios.get(`${API_URL}/api/boutiques`);
        if (data.success) setRealBoutiques(data.data);
      } catch (err) {
  ...
  // Merge display names/images with real boutique IDs, cycling through
  // whatever real boutiques exist if there are fewer than display slots.
  const BOUTIQUES = realBoutiques.length > 0
    ? BOUTIQUES_DISPLAY.map((item, idx) => ({
        ...item,
        id: realBoutiques[idx % realBoutiques.length]._id,
      }))
    : [];
