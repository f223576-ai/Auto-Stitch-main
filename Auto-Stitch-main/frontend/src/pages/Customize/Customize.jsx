...
export default function Customize() {
  ...
  useEffect(() => {
    ...
    // If Stitchie (the chatbot) finalized a customization plan, prefill the form.
    const storedPlan = sessionStorage.getItem('stitchie_plan');
    if (storedPlan) {
      try {
        const plan = JSON.parse(storedPlan);
        if (Array.isArray(plan.selectedRegions) && plan.selectedRegions.length > 0) {
          setSelectedRegions(plan.selectedRegions);
        }
        if (plan.description) setDescription(plan.description);
        if (plan.budget) setBudget(String(plan.budget));
        if (plan.selectedRegions?.length > 0) setStep(2); // skip straight to references/review
      } catch {
        // Ignore malformed/stale draft.
      } finally {
        sessionStorage.removeItem('stitchie_plan');
      }
    }
  }, [navigate]);
